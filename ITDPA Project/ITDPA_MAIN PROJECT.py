...
class AQ10App:
    def __init__(self, root):
        self.root = root
        self.root.title("🧩 SpectrumLight - Autism Pre-Screening Tool")
    ...
    def _place_pages(self):
        """Make each page stretch across the full window."""
    ...
    def _create_page1(self):
        # Create a main container with scrollbar for the entire page
        main_container = ttk.Frame(self.page1, style="TFrame")
        main_container.pack(fill="both", expand=True)

        # Create canvas and scrollbar
        self.page1_canvas = tk.Canvas(main_container, bg=FRAME_BG, highlightthickness=0, bd=0)
        scrollbar = ttk.Scrollbar(main_container, orient="vertical", command=self.page1_canvas.yview)
        self.page1_canvas.configure(yscrollcommand=scrollbar.set)

        scrollbar.pack(side="right", fill="y")
        self.page1_canvas.pack(side="left", fill="both", expand=True)

        # Create scrollable frame
        self.scrollable_frame = ttk.Frame(self.page1_canvas, style="TFrame")
        self.scrollable_window = self.page1_canvas.create_window((0, 0), window=self.scrollable_frame, anchor="nw")

        # Configure canvas scrolling
        self.page1_canvas.bind("<Configure>", self._on_page1_canvas_configure)
        self.scrollable_frame.bind("<Configure>", self._on_page1_frame_configure)

        # Bind mousewheel to canvas
        self.scrollable_frame.bind("<Enter>", lambda e: self._bind_page1_mousewheel())
        self.scrollable_frame.bind("<Leave>", lambda e: self._unbind_page1_mousewheel())

        # Now create all content inside the scrollable_frame
        container = ttk.Frame(self.scrollable_frame, padding=20, style="TFrame")
        container.pack(fill="both", expand=True)

        # App title
        ttk.Label(container, text="🧩 SpectrumLight", style="Header.TLabel").pack(anchor="w", pady=(0, 6))
        ttk.Label(container, text="Autism Pre-Screening Tool", style="Subheader.TLabel").pack(anchor="w", pady=(0, 12))

        # Warning card
        warning_card = ttk.Frame(container, padding=12, style="Card.TFrame")
        warning_card.pack(fill="x", expand=False, pady=(0, 12))

        warning_text = (
            "⚠️ IMPORTANT: This is NOT a diagnostic tool. SpectrumLight is a pre-screening assessment "
            "that can help identify traits associated with autism spectrum conditions. "
            "The results should not be used as a formal diagnosis. If you have concerns about "
            "autism, please consult with a qualified healthcare professional for a comprehensive evaluation."
        )
        warning_label = ttk.Label(warning_card, text=warning_text, style="CardText.TLabel", wraplength=900,
                                  justify="left")
        warning_label.pack(anchor="w")

        # Consent card
        consent_card = ttk.Frame(container, padding=12, style="Card.TFrame")
        consent_card.pack(fill="x", expand=False, pady=(0, 12))

        consent_text = (
            "By proceeding with this screening, you acknowledge that:\n"
            "• This is not a diagnostic tool\n"
            "• Results are for informational purposes only\n"
            "• You should consult a professional for formal assessment\n"
            "• Your data will be stored anonymously for research purposes"
        )
        consent_label = ttk.Label(consent_card, text=consent_text, style="CardText.TLabel", wraplength=900,
                                  justify="left")
        consent_label.pack(anchor="w", pady=(0, 10))

        self.consent_var = tk.BooleanVar(value=False)
        consent_check = ttk.Checkbutton(consent_card, text="I understand and agree to proceed with the screening",
                                        variable=self.consent_var, style="AQ.TRadiobutton")
        consent_check.pack(anchor="w")

        # Demographic information card
        ttk.Label(container, text="First, some basic information.", style="Subheader.TLabel").pack(anchor="w",
                                                                                                   pady=(12, 6))
        ...
        ttk.Label(card, text="Country of residence", style="CardText.TLabel").grid(row=2, column=0, sticky="w",
                                                                                   pady=(8, 0))
        ttk.Entry(card, textvariable=self.country_var, style="Entry.TEntry").grid(row=3, column=0, sticky="we",
                                                                                  padx=(0, 8), pady=(6, 6))

        ttk.Label(card, text="Jaundice (as an infant)", style="CardText.TLabel").grid(row=2, column=1, sticky="w",
                                                                                      pady=(8, 0))
        ...
        # Questions area
        ttk.Label(container, text="AQ-10 Questions", style="Subheader.TLabel").pack(anchor="w", pady=(12, 6))

        for i, q in enumerate(self.questions, start=1):
            card_q = ttk.Frame(container, padding=10, style="Card.TFrame")
            card_q.pack(fill="x", expand=True, pady=(6, 6))
            ttk.Label(card_q, text=f"{i}. {q}", style="CardTitle.TLabel", wraplength=900, justify="left").pack(
                anchor="w")
            ...
            ttk.Radiobutton(row, text="Definitely agree", value="DA", variable=var, style="AQ.TRadiobutton").pack(
                side="left", padx=(0, 12))
            ttk.Radiobutton(row, text="Slightly agree", value="SA", variable=var, style="AQ.TRadiobutton").pack(
                side="left", padx=(0, 12))
            ttk.Radiobutton(row, text="Slightly disagree", value="SD", variable=var, style="AQ.TRadiobutton").pack(
                side="left", padx=(0, 12))
            ttk.Radiobutton(row, text="Definitely disagree", value="DD", variable=var, style="AQ.TRadiobutton").pack(
                side="left")

        footer = ttk.Frame(container, padding=8, style="TFrame")
        footer.pack(fill="x", pady=(12, 0))
        ttk.Button(footer, text="Submit →", style="Primary.TButton", command=self._submit_and_show_results).pack(
            side="right", padx=(6, 0))
    ...
    def _on_page1_canvas_configure(self, event):
        # Update the scroll region to encompass the inner frame
        self.page1_canvas.configure(scrollregion=self.page1_canvas.bbox("all"))
        # Set the width of the inner window to match the canvas
        self.page1_canvas.itemconfig(self.scrollable_window, width=event.width)

    def _on_page1_frame_configure(self, event):
        # Update the scrollregion to encompass the inner frame
        self.page1_canvas.configure(scrollregion=self.page1_canvas.bbox("all"))

    def _bind_page1_mousewheel(self):
        sysname = platform.system()
        if sysname in ("Windows", "Darwin"):  # Windows & macOS
            self.page1_canvas.bind_all("<MouseWheel>", self._on_page1_mousewheel)
        else:  # Linux/X11
            self.page1_canvas.bind_all("<Button-4>", self._on_page1_mousewheel)
            self.page1_canvas.bind_all("<Button-5>", self._on_page1_mousewheel)

    def _unbind_page1_mousewheel(self):
        self.page1_canvas.unbind_all("<MouseWheel>")
        self.page1_canvas.unbind_all("<Button-4>")
        self.page1_canvas.unbind_all("<Button-5>")

    def _on_page1_mousewheel(self, event):
        sysname = platform.system()
        if sysname == "Windows":
            delta = -1 * int(event.delta / 120)
            self.page1_canvas.yview_scroll(delta, "units")
        elif sysname == "Darwin":
    ...
    def _create_page2(self):
        ...
        container.pack(fill="both", expand=True)
        ttk.Label(container, text="🧾 Personalized Screening Result", style="Header.TLabel").pack(anchor="w",
                                                                                                 pady=(0, 6))
        ttk.Label(container, text="A quick visual summary of your AQ-10 result.", style="Subheader.TLabel").pack(
            anchor="w", pady=(0, 8))
        ...
        ttk.Label(self.results_card, textvariable=self.details_var, style="CardText.TLabel", wraplength=900,
                  justify="left").pack(anchor="w", pady=(8, 6))

        actions = ttk.Frame(container, style="TFrame")
        actions.pack(fill="x")
        ttk.Button(actions, text="← Back", style="Secondary.TButton", command=lambda: self.show_page(self.page1)).pack(
            side="left")
        ttk.Button(actions, text="Next → Models", style="Primary.TButton", command=self._go_to_models_page).pack(
            side="right")

    # ---------- Page 3 (scrollable) ----------
    def _create_page3(self):
        # Main container with canvas + scrollbar
        main_container = ttk.Frame(self.page3, style="TFrame")
        main_container.pack(fill="both", expand=True)

        self.page3_canvas = tk.Canvas(main_container, bg=FRAME_BG, highlightthickness=0, bd=0)
        scrollbar = ttk.Scrollbar(main_container, orient="vertical", command=self.page3_canvas.yview)
        self.page3_canvas.configure(yscrollcommand=scrollbar.set)

        scrollbar.pack(side="right", fill="y")
        self.page3_canvas.pack(side="left", fill="both", expand=True)

        # Scrollable frame
        self.page3_scrollable = ttk.Frame(self.page3_canvas, style="TFrame")
        self.page3_window = self.page3_canvas.create_window((0, 0), window=self.page3_scrollable, anchor="nw")

        # Configure scrolling
        self.page3_canvas.bind("<Configure>", self._on_page3_canvas_configure)
        self.page3_scrollable.bind("<Configure>", self._on_page3_frame_configure)

        # Bind mousewheel
        self.page3_scrollable.bind("<Enter>", lambda e: self._bind_page3_mousewheel())
        self.page3_scrollable.bind("<Leave>", lambda e: self._unbind_page3_mousewheel())

        # Actual content container
        container = ttk.Frame(self.page3_scrollable, padding=12, style="TFrame")
        container.pack(fill="both", expand=True)

        ttk.Label(container, text="🔬 Predictive Model Comparison", style="Header.TLabel").pack(anchor="w", pady=(0, 6))
        ttk.Label(container, text="SVM vs XGBoost — ROC curves and predictions for your submission",
        ...
        ttk.Button(actions, text="Restart → Back to start", style="Secondary.TButton", command=self._restart).pack(
            side="left")
    ...
    def _on_page3_canvas_configure(self, event):
        self.page3_canvas.configure(scrollregion=self.page3_canvas.bbox("all"))
        self.page3_canvas.itemconfig(self.page3_window, width=event.width)

    def _on_page3_frame_configure(self, event):
        self.page3_canvas.configure(scrollregion=self.page3_canvas.bbox("all"))

    def _bind_page3_mousewheel(self):
        sysname = platform.system()
        if sysname in ("Windows", "Darwin"):
            self.page3_canvas.bind_all("<MouseWheel>", self._on_page3_mousewheel)
        else:
            self.page3_canvas.bind_all("<Button-4>", self._on_page3_mousewheel)
            self.page3_canvas.bind_all("<Button-5>", self._on_page3_mousewheel)

    def _unbind_page3_mousewheel(self):
        self.page3_canvas.unbind_all("<MouseWheel>")
        self.page3_canvas.unbind_all("<Button-4>")
        self.page3_canvas.unbind_all("<Button-5>")

    def _on_page3_mousewheel(self, event):
        sysname = platform.system()
        if sysname == "Windows":
            delta = -1 * int(event.delta / 120)
            self.page3_canvas.yview_scroll(delta, "units")
        elif sysname == "Darwin":
            delta = -1 if event.delta > 0 else 1
            self.page3_canvas.yview_scroll(delta, "units")
        else:
    ...
    def _append_to_dataset(self, row_dict):
        ...
        ordered_cols = [c for c in REQUIRED_HEADERS] + [c for c in EXTRA_HEADERS] + [c for c in df_now.columns if
                                                                                     c not in REQUIRED_HEADERS + EXTRA_HEADERS]
    ...
    def _submit_and_show_results(self):
        ...
        if not self.consent_var.get():
            messagebox.showwarning("Consent Required",
                                   "Please acknowledge the consent statement to proceed with the screening.")
            return
        ...
        if self.gender_var.get() not in ("Male", "Female", "Other"):
            errors.append("Please select a gender (Male, Female, or Other) on Page 1.")
        if self.ethnicity_var.get() not in (
                "White-European", "Black", "Asian", "Latino", "Middle Eastern", "South Asian", "Hispanic", "Pasifika",
                "Other"):
    ...
    def _display_results_page(self, aq10_total, details, screening_class):
        ...
        if MATPLOTLIB_AVAILABLE:
            ...
            ttk.Label(self.chart_holder, text=f"Your AQ-10 Score: {aq10_total}/10 (Threshold: 6)",
                      style="CardText.TLabel").pack(anchor="center", pady=20)
    ...
    def _train_and_display_models(self):
        ...
        if Xdf.empty or len(y) < 10:
            messagebox.showwarning("Not enough data",
                                   "Not enough labeled rows to train models. Need at least ~10 labeled rows.")
            return
        ...
        if MATPLOTLIB_AVAILABLE:
            ...
            if "error" not in svm_metrics and "cm" in svm_metrics:
                ...
                ax_cm_svm.set_xticks([0, 1]);
                ax_cm_svm.set_xticklabels(["Non-ASD", "ASD"])
                ax_cm_svm.set_yticks([0, 1]);
                ax_cm_svm.set_yticklabels(["Non-ASD", "ASD"])
            ...
            if "error" not in xgb_metrics and "cm" in xgb_metrics:
                ...
                ax_cm_xgb.set_xticklabels(["Non-ASD", "ASD"])
                ax_cm_xgb.set_yticks([0, 1]);
                ax_cm_xgb.set_yticklabels(["Non-ASD", "ASD"])
                for (i, j), v in np.ndenumerate(cm):
            ...
            ttk.Label(self.models_plot_holder, text="matplotlib not installed — plots unavailable",
                      style="CardText.TLabel").pack()
        ...
        if self.latest_submission is None:
            output_lines.append(
                "No recent submission found to predict. Return to the questionnaire and submit first.\n")
        else:
